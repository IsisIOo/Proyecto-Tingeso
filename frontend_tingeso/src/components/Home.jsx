const Home = () => {
  return (
    <div>
      <h1>Proyecto Tingeso</h1>
      <p>
        El presente proyecto es una aplicación web para gestionar costos de reparaciones a vehículos. 
        Esta aplicación ha sido desarrollada usando tecnologías como{" "}
        <a href="https://spring.io/projects/spring-boot">Spring Boot</a> (para
        el backend), <a href="https://reactjs.org/">React</a> (para el Frontend)
        y <a href="https://www.postgresql.org/about/">PostgreSQL</a> (para la
        base de datos).
      </p>
    </div>
    
  );
};

export default Home;
