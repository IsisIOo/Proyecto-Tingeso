...
const AddEditCost = () => {
    ...
    e.preventDefault();
        ...
        if (id) {
          ...
          costService
            .CALCULATODO(patent)
            .then((response) => {
              console.log("Ticket ha sido añadido.", response.data);
              navigate("/cost/list");
            })
            .catch((error) => {
              console.log(
                "Ha ocurrido un error al intentar crear nuevo Ticket.",
                error
              );
            });
        }
  ...
  return (
    ...
    >
      ...
      <hr />
      <Link to="/cost/list">Back to List</Link>
    </Box>
    </Paper>
  );
};
